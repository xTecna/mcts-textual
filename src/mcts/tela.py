...
from mcts.jogo import JogadaJogoDaVelha, JogadaLig4, JogoDaVelha, Lig4
...
class TelaLig4(App[None]):
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.linhas = linhas
        self.casas_para_ganhar = casas_para_ganhar
        self.jogo = Lig4(colunas, linhas, casas_para_ganhar)
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        self.title = 'Jogando o lig 4 com a IA'
    ...
    async def set_start(self) -> None:
        ...
        self.jogo = Lig4(self.colunas, self.linhas, self.casas_para_ganhar)
        self.set_tree(None)
        await self.set_table()
        self.elemento_game_result.update('')
        self.elemento_ia_info.update(f'Iteracoes por jogada: {self.max_iteracoes}')
    ...
    async def set_table(self) -> None:
        self.elemento_game_table.clear()
        self.elemento_game_table.cursor_type = 'column'

        columns = list(self.elemento_game_table.columns.keys())
        for column in columns:
            self.elemento_game_table.remove_column(column)
        coordenadas = [str(i) for i in range(self.colunas)]
        self.elemento_game_table.add_columns(*coordenadas)

        for _ in range(self.linhas):
            linha_tabela = [self.simbolos[None] for _ in range(self.colunas)]
            self.elemento_game_table.add_row(*linha_tabela)

    def _update_column(self, column: int) -> None:
        for linha, valor in enumerate(reversed(self.jogo.tabuleiro[column])):
            self.elemento_game_table.update_cell_at(Coordinate(linha, column), self.simbolos[valor])

    @on(DataTable.ColumnSelected, '#game-table')
    async def choose_column(self, event: DataTable.ColumnSelected) -> None:
        self.elemento_game_table.disabled = True

        self.pausado = True
        self.refresh_bindings()

        jogada = event.cursor_column

        try:
            self.jogo.joga(jogada)
            self._update_column(jogada)
        except:
            self.pausado = False
            self.refresh_bindings()
    ...
    def ia_play(self) -> None:
        ia = IA[JogadaLig4](self.c, self.max_iteracoes)
        jogada = ia.escolhe_jogada(self.jogo)
        self.jogo.joga(jogada)
        self.set_tree(ia.arvore)
    ...
    def set_tree(self, arvore: No[JogadaLig4] | None) -> None:
        self.elemento_ia_tree.clear()

        if arvore:
            fila: Queue[tuple[TreeNode[None], No[JogadaLig4]]] = Queue()
            fila.put((self.elemento_ia_tree.root, arvore))
            ...
            while not fila.empty():
                ...
                for jogada in sorted(no_atual.filhos):
                    ...
                    visitas_pai = filho.pai.visitas if filho.pai else 0
                    child_element = element.add(
                        f'{jogada}) {filho.visitas} {filho.pontuacao:.6f} [{filho.pontuacao:.6f} + {self.c} * sqrt({visitas_pai} / {filho.visitas})]',
                        expand=False,
                    )
